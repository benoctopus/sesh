...
pub fn get_upstream(repo: &Repository, branch: &str) -> Result<String> {
    ...
    upstream.name()
        .ok_or_else(|| Error::GitError {
            message: "Upstream branch has invalid name".to_string(),
        })
        .map(|s| s.to_string())
}

/// Check if a remote branch exists
pub async fn remote_branch_exists(repo: &Repository, upstream: &str) -> Result<bool> {
    ...
    .await
    .map_err(|e| Error::GitError {
        message: format!("Remote branch check task failed: {}", e),
    })?
}
